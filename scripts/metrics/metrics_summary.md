...
| `ACR` | 资产覆盖率     | 已实现         | 需要每条任务的`required_assets` 标注                                                        |
| `RSR` | 空间关系满足率 | 检查支撑关系  | 需要`required_relations` 标注，并实现空间关系 checker                                       |
...
| `TCA` | 工具调用准确率  训练时候导出  最后的 | 已实现   | 需要专家 trace 或 gold tool sequence            |
