...
* **单位与坐标系 (Units & Coordinate System):**
  ...
  * **Up Axis:**Z
* **场景描述(Scene Describe):** 该工业场景呈现为一个极具规模感的大型矩阵式自动化分拣与装卸车间，其空间格局采用了“多级并行、矩阵分布”的高密度规划逻辑，旨在应对超大规模标准化物流包裹的瞬时吞吐与柔性调拨。车间内部视野极度开阔，呈现出一种深远且规整的几何韵律。地面铺设了大面积具有灰色大理石质感的高级地砖，纹理细腻且带有微弱的镜面反射效果，不仅赋予了工业环境一种严谨的精密感，也为设备运行提供了极佳的平整度支撑；侧向墙体采用了带有垂直分区的灰色工业挂板及大幅采光窗设计，增强了空间的纵深感。在设备排布方面，场景的核心由呈矩阵状交错排布的多条长程带式输送机与高密度的协作单元组成。可见至少四至五条平行的输送流水线横向贯穿作业区，每条输送线均采用银灰色的金属框架结构，配以纯黑色的抗静电输送皮带。输送带表面等间距排布着数十个浅棕色标准纸箱，包裹数量之多、密度之大，展现了极高的自动化流水水平。与每条输送线逻辑配套的是一套分布式的“机器人+AGV”执行阵列。在输送线的各功能节点处，密集部署了大量的白色六轴协作机器人，这些机器人采用纤细的长臂设计，底座固定于地面，机身呈现出半哑光漆面的现代质感。它们呈线性队列排布，精准覆盖了输送线的末端出口及侧向分拨点。与之实时对接的是一组活跃在通道内的自主移动机器人（AGV），这些AGV平台采用醒目的亮黄色涂装，底盘低矮且具备全向移动能力，顶部承载区域堆叠有两至三个标准包裹。这种“一机多点、一单元一AGV”的网格化协作模式，构成了场景中极其复杂的物流拓扑关系。设备数量识别清晰：输送机为阵列式分布的多组单元，机器人数量超过十台，AGV移动终端亦呈阵列式分布。相互排布关系呈现出极强的空间逻辑：输送机作为物料的长程运输载体，与机器人的垂直抓取路径及AGV的水平运动轨迹交织在一起，形成了一个立体的自动化分拣网络。这种规划结构彻底打破了传统单线作业的瓶颈，通过矩阵式的并行处理极大地提升了系统的冗余性与作业效率。在3D场景合成中，该场景提供了极丰富的资产复用价值与极致的对齐参考，材质上涵盖了银灰色喷漆金属、白色半哑光机器人外壳、明黄色高亮度反光漆面以及石材地面纹理的多维对比。光影环境设定为全局均匀的漫反射高空照明，配合侧向窗户引入的微弱环境光，使场景呈现出极强的空间层次感与写实的物理深度，是构建超大规模智慧分拣中心与未来工厂数字孪生场景的理想专业素材基准。
* **设备数量(Device Number):**
  * **Scene: 1**
  * **IndustrialRobot: 16**
  * **Workbench: 0**
