...
try:
    from .bbox_coordinates import normalize_bbox_to_z_up_meters
except ImportError:
    from bbox_coordinates import normalize_bbox_to_z_up_meters
...
BBOX_MIN_COLUMN = "bbox_min_meters"
BBOX_MAX_COLUMN = "bbox_max_meters"
...
def get_bbox_in_meters(
    ...
    """Compute an aligned bbox in the canonical Z-up, meter-based frame."""
    ...
    return normalize_bbox_to_z_up_meters(
        bbox,
...
def process_csv(csv_file: Path, output_suffix: str) -> Optional[Path]:
    ...
    source_text = csv_file.read_text(encoding="utf-8-sig")
    first_line_end = source_text.find("\n")
    source_lineterminator = (
        "\r\n"
        if first_line_end > 0 and source_text[first_line_end - 1] == "\r"
        else "\n"
    )
    source_has_final_newline = source_text.endswith(("\r", "\n"))
    rows = list(csv.reader(io.StringIO(source_text, newline="")))
    ...
    for bbox_column in (BBOX_MIN_COLUMN, BBOX_MAX_COLUMN):
        if bbox_column not in output_header:
            output_header.append(bbox_column)
    bbox_min_index = output_header.index(BBOX_MIN_COLUMN)
    bbox_max_index = output_header.index(BBOX_MAX_COLUMN)

    output_rows = []

    for index, row in enumerate(data_rows, start=1):
        normalized_row = list(row)
        if len(normalized_row) < len(output_header):
            normalized_row.extend([""] * (len(output_header) - len(normalized_row)))

        raw_path = (
            normalized_row[path_column_index]
            if path_column_index < len(normalized_row)
            else ""
        )
        usd_path = _normalize_usd_path(raw_path, csv_file)
        if usd_path is None:
            normalized_row[bbox_min_index] = ""
            normalized_row[bbox_max_index] = ""
        ...
        try:
            ...
            normalized_row[bbox_min_index] = str(bbox_min_meters)
            normalized_row[bbox_max_index] = str(bbox_max_meters)
            output_rows.append(normalized_row)
    ...
    writer = csv.writer(output_buffer, lineterminator=source_lineterminator)
    writer.writerow(output_header)
    writer.writerows(output_rows)
    output_text = output_buffer.getvalue()
    if not source_has_final_newline:
        output_text = output_text.removesuffix(source_lineterminator)
    output_file.write_text(output_text, encoding="utf-8-sig", newline="")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="读取一个或多个 CSV，计算 USD 路径对应的 Z-up 米制 bbox，并写入结果 CSV。",
    )
    parser.add_argument(
        "--inputs",
        nargs="+",
